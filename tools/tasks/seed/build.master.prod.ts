 ...
 */
export = () => {
  return gulp.src(join(Config.APP_SRC, 'custom_angular2_master.html'))
    .pipe(injectJs())
    .pipe(injectCss())
...
function inject(...files: Array<string>) {
    return plugins.inject(gulp.src(files, { read: false }), {
      files,
      starttag: Config.CUSTOM_CONFIG.gulp_inject_config.starttag,
      endtag: Config.CUSTOM_CONFIG.gulp_inject_config.endtag,
...
function transformPath() {
  return function(filepath: string) {
    ...
    if (path.indexOf('.js') > 0 && Config.ENV === 'prod') {
    ...
    if (path.indexOf('.css') > 0 && Config.ENV === 'prod') {
