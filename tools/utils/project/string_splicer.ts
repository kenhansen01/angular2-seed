 ...
 * @param {string} findString - the string preceding or following the string to add,
 * this should be a unique string in the original (not repeated)
...
export function StringSplicer(
  ...
  let newString = addAfter ?
    `${ogSplitArray[0]} ${findString} ${addString} ${ogSplitArray[1]}`:
    `${ogSplitArray[0]} ${addString} ${findString} ${ogSplitArray[1]}`;
  return newString;
}
